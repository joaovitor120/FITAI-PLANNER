...
const FOOD_KCAL_PER_G = [
  { key: 'arroz', kcal: 1.30, p: 0.026, c: 0.282, f: 0.003 },
  { key: 'feijao', kcal: 0.76, p: 0.048, c: 0.137, f: 0.005 },
  { key: 'frango', kcal: 1.65, p: 0.31, c: 0, f: 0.036 },
  { key: 'banana', kcal: 0.89, p: 0.011, c: 0.23, f: 0.003 },
  { key: 'aveia', kcal: 3.89, p: 0.17, c: 0.66, f: 0.07 },
  { key: 'iogurte', kcal: 0.61, p: 0.035, c: 0.047, f: 0.033 },
  { key: 'granola', kcal: 4.71, p: 0.10, c: 0.64, f: 0.20 },
  { key: 'pao', kcal: 2.65, p: 0.09, c: 0.49, f: 0.03 },
  { key: 'queijo', kcal: 2.64, p: 0.17, c: 0.02, f: 0.20 },
  { key: 'batata doce', kcal: 0.86, p: 0.016, c: 0.20, f: 0.001 },
  { key: 'macarrao', kcal: 1.58, p: 0.058, c: 0.30, f: 0.009 },
  { key: 'atum', kcal: 1.16, p: 0.25, c: 0, f: 0.01 },
  { key: 'castanhas', kcal: 6.00, p: 0.18, c: 0.17, f: 0.52 },
  { key: 'leite', kcal: 0.62, p: 0.032, c: 0.048, f: 0.033 },
  { key: 'mandioca', kcal: 1.25, p: 0.01, c: 0.30, f: 0.003 },
  { key: 'peixe', kcal: 1.28, p: 0.24, c: 0, f: 0.03 },
  { key: 'omelete', kcal: 1.80, p: 0.12, c: 0.02, f: 0.14 },
  { key: 'ovos', kcal: 1.55, p: 0.13, c: 0.01, f: 0.11 },
  { key: 'azeite', kcal: 8.84, p: 0, c: 0, f: 1.0 },
  { key: 'salada', kcal: 0.20, p: 0.01, c: 0.03, f: 0.002 },
  { key: 'legumes', kcal: 0.35, p: 0.015, c: 0.07, f: 0.003 },
  { key: 'sopa', kcal: 0.45, p: 0.02, c: 0.06, f: 0.01 }
];

function estimateItemNutrition(itemText = '') {
  ...
  const qty = grams || (mlMatch ? Number(mlMatch[1]) : null);
  const food = FOOD_KCAL_PER_G.find(f => n.includes(f.key));
  if (!food || !qty) return { calories: null, protein_g: 0, carbs_g: 0, fat_g: 0 };
  return {
    calories: Math.round(food.kcal * qty),
    protein_g: Math.round(food.p * qty),
    carbs_g: Math.round(food.c * qty),
    fat_g: Math.round(food.f * qty)
  };
}
...
function generateWorkout(profile, checkin) {
  ...
  const libHomeDumbbell = {
    push: ['Flexão', 'Desenvolvimento halter', 'Mergulho banco', 'Elevação lateral', 'Tríceps testa', 'Supino com halteres no chão'],
    pull: ['Remada unilateral halter', 'Pullover', 'Rosca martelo', 'Encolhimento', 'Remada curvada halter', 'Face pull com elástico'],
  ...
  const libHomeBodyweight = {
    push: ['Flexão', 'Flexão inclinada', 'Mergulho banco', 'Flexão arqueiro assistida', 'Prancha com toque no ombro'],
    pull: ['Remada australiana (mesa/barra)', 'Puxada com toalha isométrica', 'Remada invertida sob mesa', 'Pulldown com elástico', 'Rosca com mochila'],
    legs: ['Agachamento livre', 'Afundo', 'Elevação pélvica', 'Panturrilha em pé', 'Agachamento isométrico'],
    full: ['Agachamento livre', 'Flexão', 'Remada invertida sob mesa', 'Elevação pélvica', 'Prancha']
  };
...
app.get('/api/workouts/current', auth, async (req, res) => {
  ...
  const profile = await get('SELECT equipment FROM profiles WHERE user_id = ?', [req.user.id]);
  res.json({ ...workout, equipment: profile?.equipment || '', plan: JSON.parse(workout.plan_json) });
});
  ...
  try {
    ...
    try {
      analysis = await geminiJson({
        systemPrompt: 'Você é um coach de treino avançado e prático. Responda com alta especificidade, cite ajustes objetivos (exercícios, volume, intensidade, descanso), evite generalidades e nunca mencione funcionamento interno do sistema.',
        userPrompt: `Perfil=${JSON.stringify(profile || {})}\nTreino=${JSON.stringify(plan)}\nFeedback=${message}`,
        schemaHint: '{"intent":"question|reduce|increase|rebalance","target":"full|legs|chest|upper|lower","deltaSets":-1,"newRir":"RIR 2-3","rationale":"texto técnico curto","coachReply":"resposta final para o usuário","summary":"o que será atualizado para qual atualização"}'
      });
    } catch {
      ...
      analysis = {
        ...
        rationale: 'Ajuste técnico com base em dor, fadiga, recuperação e percepção de esforço.',
        coachReply: 'Analisei seu feedback e montei um ajuste técnico para manter evolução com segurança.',
        summary: 'o volume das séries e a intensidade (RIR) conforme seu feedback'
      };
    }
...
app.post('/api/nutrition/plan', auth, async (req, res) => {
  try {
    ...
    const selectedMealPool = mealPool.slice(0, Math.min(Math.max(Number(meals_count) || 4, 3), 6));
    const mealCaloriesDefault = Math.round(t.calories / selectedMealPool.length);
    const filteredMeals = selectedMealPool.map(m => {
      ...
      const mealCarbs = items.reduce((acc, it) => acc + (it.carbs_g || 0), 0);
      const mealFat = items.reduce((acc, it) => acc + (it.fat_g || 0), 0);
  ...
  try {
    ...
    try {
      reply = await geminiText({
        systemPrompt: 'Você é nutricionista esportivo virtual com foco em personalização. Responda sem generalidades, com opções práticas em gramas/unidades e impacto em kcal, proteína, carboidrato e gordura. Seja objetivo e útil para qualquer dúvida de dieta.',
      ...
      reply = `Para seu contexto atual, recomendo ${goal}.\n\nMeta atual: ${plan.calories} kcal | P ${plan.protein_g}g | C ${plan.carbs_g}g | G ${plan.fat_g}g.\n\nSe quiser, diga a refeição exata (café/almoço/lanche/jantar) e eu te devolvo substituições em gramas.`;
    }
